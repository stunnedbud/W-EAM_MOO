...
fname_txt = "/home/arivera/weam/EMNIST/ExecutionTimeLogs.txt"
fname_csv = "/home/arivera/weam/EMNIST/ExecutionTimeResultChart.csv"
...
smac_outdir = "SMS-EMOA_tenfold_results"
# Historico de configuraciones intentadas
statsfilename = smac_outdir + "/EMNIST_configurations.csv"
...
def evaluate_memory_config(
    ...
    # Las cotas de parametros seran implementadas aqui por la función de optimización
    ...
    presum.append(precision_sum)
    recsum.append(recall_sum)
    presum_avg = sum(presum) / len(presum)
    recsum_avg = sum(recsum) / len(recsum)
    return presum_avg, recsum_avg
...
problem = Problem(
    evaluate_memory_config, num_objectives=2, max_evaluations=1, name="WEAM_EMNIST"
)
...
end_time = time.time()
log(
    f"Ended EMNIST SMSEMOA Run with max_evaluations = 1. Total execution time: {end_time - start_time:.2f} seconds",
    fname_txt,
)
...
start_time = time.time()
problem = Problem(
    evaluate_memory_config, num_objectives=2, max_evaluations=10, name="WEAM_EMNIST"
)
dim = 4  # parametro para sbx
popsize = 20
population = []
