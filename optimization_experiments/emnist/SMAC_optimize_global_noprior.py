...
fname_txt = "/home/arivera/smac_runtimes/EMNIST_global_ExecutionTimeLogs.txt"
fname_csv = "/home/arivera/smac_runtimes/EMNIST_global_ExecutionTimeResultChart.csv"
...
statsfilename = smac_outdir + "/EMNIST_configs_global_noprior.csv"
...
# for j in range(n_memories):
config.append(
    UniformIntegerHyperparameter("tolerance", lower=0, upper=maxtol)
)  # (1, 10), default=4))
# config.append(UniformFloatHyperparameter(str(j)+"_sigma", lower=0.0, upper=hl, default_value=prior_sigma))
config.append(UniformFloatHyperparameter("iota", lower=0.0, upper=hl))
config.append(UniformFloatHyperparameter("kappa", lower=0.0, upper=hl))
config.append(UniformIntegerHyperparameter("memory_size", lower=0, upper=maxmemsize))
...
def evaluate_memory_config(
    ...
    log(
        f"Ended objective function call. Total execution time: {end_time_obj - start_time_obj:.2f} seconds",
        fname_txt,
    )
    return score
