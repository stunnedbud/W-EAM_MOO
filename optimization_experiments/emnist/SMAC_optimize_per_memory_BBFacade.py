...
fname_txt = "/home/arivera/smac_runtimes/EMNIST_per_memory_ExecutionTimeLogs.txt"
fname_csv = "/home/arivera/smac_runtimes/EMNIST_per_memory_ExecutionTimeResultChart.csv"
...
statsfilename = smac_outdir + "/EMNIST_noprior_configurations_statistics_per_memory.csv"
...
# Generamos una configuración para cada memoria
config = {}
for j in range(n_memories):
    config[str(j) + "_tolerance"] = (0, maxtol)
    # config[str(j)+"_sigma"] = (0.0, hl)
    config[str(j) + "_iota"] = (0.0, hl)
    config[str(j) + "_kappa"] = (0.0, hl)
    config[str(j) + "_memory_size"] = (1, maxmemsize)
...
def get_wams_results(
    ...
    behaviour = np.zeros(
        (nmems, constants.n_behaviours), dtype=np.float64
    ...
    # Recognition
    response_size = np.zeros(nmems, dtype=int)
