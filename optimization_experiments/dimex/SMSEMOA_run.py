...
fname_txt = "SMSEMOA_DMX_ExecutionTimeLogs.txt"
fname_csv = "SMSEMOA_DMX_ExecutionTimeResultChart.csv"
...
statsfilename = smac_outdir + "/DIMEX_configurations.csv"
...
if not os.path.exists(os.path.join(os.getcwd(), statsfilename)):
    with open(os.path.join(os.getcwd(), statsfilename), "w+") as outf:
        outf.write(
            "OBJ_1 = (-1)*Suma de Precisions, OBJ_2 = (-1)*Suma de Recalls, F1 (Promedio), F1 (desviación estándar), Tamaño de memoria, Tolerancia, Sigma, Iota, Kappa, Precisión (promedio), Precisión (desviación estándar), Recall (promedio), Recall (desviación estándar), Entropía (promedio), Entropía (desviación estándar)\n"
        )
...
def get_ams_results(config, domain, trf, tef, trl, tel, fold):
    ...
    # Round the values
    # msize = max_mem_size # para normalizar entre el maximo tamaño de memoria
    max_value = trf.max()
    other_value = tef.max()
    ...
    return ((-1) * (precision_sum), (-1) * (recall_sum), None, behaviour, cms)
...
def evaluate_memory_config(
    ...
    iota = config[0]
    kappa = config[1]
    tolerance = int(math.floor(config[2]))

    # Las cotas de parametros seran implementadas por la función de optimización
    if iota < 0.0 or kappa < 0.0 or tolerance < 0.0:  # msize < 0.0
        return 0.0, 0.0
    if iota > 10 or kappa > 10.0 or tolerance > 64:  # or msize > 300.0
        return 0.0, 0.0
    # msize = int(math.floor(msize))

    configDict = (
        {}
    )  # para mantener el formato usado en smac, parseamos arreglo config en un diccionario
    for j in range(n_labels):
        configDict[str(j) + "_iota"] = iota
        configDict[str(j) + "_kappa"] = kappa
        configDict[str(j) + "_tolerance"] = tolerance

    precision_sum, recall_sum, _, _, _ = get_ams_results(
        configDict,
        domain,
        filling_features,
        testing_features,
...
problem = Problem(
    evaluate_memory_config, num_objectives=2, max_evaluations=1, name="SMS_DIMEX"
)
dim = 3  # parametro para sbx
popsize = 20
population = []
...
end_time = time.time()
log(
    f"Ended EMNIST SMSEMOA DMX Run with max_evaluations = 1. Total execution time: {end_time - start_time:.2f} seconds",
    fname_txt,
)
...
start_time = time.time()
problem = Problem(
    evaluate_memory_config, num_objectives=2, max_evaluations=10, name="SMS_DIMEX"
)
dim = 3  # parametro para sbx
popsize = 20
population = []
...
end_time = time.time()
log(
    f"Ended SMSEMOA DMX Run with max_evaluations = 10. Total execution time: {end_time - start_time:.2f} seconds",
    fname_txt,
)
